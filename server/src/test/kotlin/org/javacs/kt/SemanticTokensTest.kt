...
class SemanticTokensTest : SingleFileTestFixture("semantictokens", "SemanticTokens.kt") {
    ...
    fun `tokenize file`() {
        ...
        val expectedString = sequenceOf(
            SemanticToken(
                range(stringLine, 5, stringLine, 11),
                SemanticTokenType.PROPERTY,
                setOf(SemanticTokenModifier.DECLARATION, SemanticTokenModifier.READONLY)
            ), // string
        )
        ...
        val partialResponse = languageServer.textDocumentService.semanticTokensRange(
            semanticTokensRangeParams(
                file,
                range(constLine, 0, classLine + 1, 0)
            )
        ).get()!!
        assertThat(partialResponse.data, contains(*partialExpected.toTypedArray()))

        val fullExpected =
            encodeTokens(expectedVar + expectedConst + expectedString + expectedClass + expectedFun + expectedEnum)
