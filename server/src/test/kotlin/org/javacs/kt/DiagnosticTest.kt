...
class DiagnosticTest : SingleFileTestFixture("diagnostic", "Diagnostics.kt") {
    @Test
    fun `report diagnostics on open`() {
    ...
    fun `linting should not be dropped if another linting is running`() {
        ...
        languageServer.textDocumentService.lintRecompilationCallback = {
            if (callbackCount++ == 0) {
                ...
                languageServer.textDocumentService.documentSymbol(DocumentSymbolParams(TextDocumentIdentifier(uri(file).toString())))
                    .get()
            }
        }
