...
fun getStoragePath(params: InitializeParams): Path? {
    params.initializationOptions?.let { initializationOptions ->
        ...
        val options =
            gson.fromJson(
                initializationOptions as JsonElement,
                InitializationOptions::class.java
            )
...
data class InitializationOptions(
    // A path to a directory used by the language server to store data. Used for caching
    // purposes.
