...
private fun elementCompletionItems(
    ...
    val sorted = if (partial.length >= MIN_SORT_LENGTH) {
        completions.sortedBy { stringDistance(name(it), partial) }
    } else {
...
private fun completionItem(
    ...
    val result = descriptor.accept(RenderCompletionItem(renderWithSnippets), null)

    result.label = methodSignature.find(result.detail)?.groupValues?.get(1) ?: result.label

    if (isNotStaticJavaMethod(descriptor) && (isGetter(descriptor) || isSetter(descriptor))) {
        val name = extractPropertyName(descriptor)
