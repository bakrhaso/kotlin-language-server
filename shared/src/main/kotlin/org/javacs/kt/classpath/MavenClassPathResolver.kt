...
internal class MavenClassPathResolver private constructor(private val pom: Path) : ClassPathResolver {
    ...
    override val classpath: Set<ClassPathEntry>
        get() {
            ...
            return artifacts.mapNotNull { findMavenArtifact(it, false)?.let { it1 -> ClassPathEntry(it1, null) } }
                .toSet()
        }
        ...
        get() {
            ...
            return artifacts.mapNotNull {
                findMavenArtifact(it, false)?.let { it1 ->
                    ClassPathEntry(it1, if (it.source) findMavenArtifact(it, it.source) else null)
                }
            }.toSet()
        }
...
private fun generateMavenDependencyList(pom: Path): Path {
    ...
    val command = listOf(
        mvnCommand(pom).toString(),
        "dependency:list",
        "-DincludeScope=test",
        "-DoutputFile=$mavenOutput",
        "-Dstyle.color=never"
    )
